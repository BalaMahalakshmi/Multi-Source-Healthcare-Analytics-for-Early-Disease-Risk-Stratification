# ============================================================
# STREAMLIT DASHBOARD — FULL VERSION (app.py)
...
# ════════════════════════════════════════════════════════════
    ...
    if os.path.exists("data/batch_predictions.csv"):
        ...
        st.dataframe(view.style.applymap(color_risk, subset=["risk_category"]),
...
# ════════════════════════════════════════════════════════════
    ...
    with rc1:
        ...
        h_cat   = st.selectbox("Heart Category", cats, index=cats.index(hr["risk_category"]) if hr["risk_category"] in cats else 1)
    with rc2:
        st.markdown("**Diabetes**")
        d_score = st.slider("Diabetes Risk %", 0.0, 100.0, float(dia["risk_score_pct"]))
        d_cat   = st.selectbox("Diabetes Category", cats, index=cats.index(dia["risk_category"]) if dia["risk_category"] in cats else 1)
